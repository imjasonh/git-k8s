...
func main() {
	...
	// Resolver controller uses shallow clones initially but deepens as needed
	// for 3-way merge operations.
	cacheDir := os.Getenv("GIT_CACHE_DIR")
	wsMgr := workspace.NewManager(cacheDir, true /* shallow */)
	ctx = workspace.WithManager(ctx, wsMgr)

	sharedmain.MainWithContext(ctx, "resolver-controller", resolver.NewController)
}
